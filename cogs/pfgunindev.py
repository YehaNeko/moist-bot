...
class PfgunIndev(commands.Cog):
    ...
    async def pfgunindev(
        ...
        """ Save to cache """
        if ctx.invoked_with == "pfgunindev":  # Check if the command wasn't invoked by a subcommand
            cache_arg(ctx.args[1:])  # Args without command object

    @pfgunindev.command(clean_params=True)
    ...
    @pfgunindev.command(aliases=["last", "clean", "prev", "previous", "unmod", "unmodified"], clean_params=True)
    ...
    @pfgunindev.command(aliases=["m", "multiplier"], clean_params=True)
...
def setup(client):
    client.add_cog(PfgunIndev(client))
